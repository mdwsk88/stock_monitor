...
public class WebFetchTool {
     ...
     * @return 网页的原始HTML内容
     */
    //@Tool(description = "获取指定URL网页的原始HTML内容，不进行Markdown转换")
    public String fetchRawHtml(
            @ToolParam(description = "需要获取HTML内容的网页URL地址") String url) {
        log.info("========== 调用MCP工具：fetchRawHtml() ==========");
        log.info("| URL: {}", url);
        ...
        try {
            ...
            log.info("| 成功获取HTML内容，长度: {} 字符", html != null ? html.length() : 0);
            
            return html != null ? html : "错误：响应内容为空";
