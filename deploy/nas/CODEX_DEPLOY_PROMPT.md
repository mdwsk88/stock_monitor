...
- 部署目录: /volume1/docker/stock-monitor
- Docker 数据目录: /volume1/docker
- compose 文件:
...
- NAS 已开启 SFTP，端口同样是 <nas-ssh-port>；如果手动传文件，可直接用 scp/sftp，异常时再回退 tar-over-ssh。
- 不要打印 env 文件中的密钥或密码。
- 部署完成后检查：
...
- 两个服务已经在 NAS 上跑通过一次，目录是 `/volume1/docker/stock-monitor`
- `stock-monitor-web` 和 `stock-monitor-a-stock-mcp` 当前都能启动并通过健康检查
- `a-stock-mcp` 的公网地址 `http://<public-host>:8091/sse` 已验证可完成 SSE 建连、`initialize`、`tools/list`
